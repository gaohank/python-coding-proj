...
def conv_main(data, units, filters, workspace):
    ...
    for i in range(len(units)):
        ...
        body = mx.sym.Convolution(data=body, weight=_weight, bias=_bias, num_filter=f, kernel=(3, 3), stride=(2, 2),
                                  pad=(1, 1),
        ...
        for j in range(units[i]):
            ...
            _body = mx.sym.Convolution(data=body, weight=_weight, no_bias=True, num_filter=f, kernel=(3, 3),
                                       stride=(1, 1), pad=(1, 1),
            ...
            idx += 1
            _weight = mx.symbol.Variable("conv%d_%d_weight" % (i + 1, idx), init=mx.init.Normal(0.01))
            _body = mx.sym.Convolution(data=_body, weight=_weight, no_bias=True, num_filter=f, kernel=(3, 3),
                                       stride=(1, 1), pad=(1, 1),
                                       name="conv%d_%d" % (i + 1, idx), workspace=workspace)
            _body = mx.sym.LeakyReLU(data=_body, act_type='prelu', name="relu%d_%d" % (i + 1, idx))
            idx += 1
