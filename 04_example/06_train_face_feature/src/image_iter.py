...
class FaceImageIterList(io.DataIter):
    ...
    def next(self):
        # self.cur_iter = self.iter_list[self.cur_iter_num]
        # while True:
        #    try:
        #        ret = self.cur_iter.next()
        #    except StopIteration:
        #        self.cur_iter.reset()
        #        print("[INFO]:path {} training completed ...".format(self.cur_iter.path_imgrec))
        #        if self.cur_iter_num+1 < len(self.iter_list):
        #            self.cur_iter_num += 1
        #        else:
        #            self.cur_iter_num = 0
        #            raise StopIteration
